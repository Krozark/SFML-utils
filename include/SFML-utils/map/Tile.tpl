...
namespace sfutils
{
    namespace map
    {
        ...
        inline sf::Vector2f Tile<GEOMETRY>::mapPixelToCoords(float x,float y,float scale)
        {
            return GEOMETRY::mapPixelToCoords(x,y,scale);
        }

        template<typename GEOMETRY>
        inline sf::Vector2f Tile<GEOMETRY>::mapPixelToCoords(const sf::Vector2f& pos,float scale)
        {
